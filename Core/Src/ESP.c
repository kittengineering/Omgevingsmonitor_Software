...
static bool ESP_Receive(uint8_t* reply, uint8_t length) { //Possible problem?
...
void ESP_Upkeep(void) {
  switch (EspState) {
    case ESP_STATE_OFF:
      // Turning off the ESP
//      HAL_GPIO_WritePin(Wireless_EN_GPIO_Port, Wireless_EN_Pin, GPIO_PIN_RESET); //LDO Pin?
      EspTurnedOn = false;
    ...
    case ESP_STATE_INIT:
      ...
      StartUpDone = false;
      if(!EspTurnedOn) {
//        HAL_GPIO_WritePin(Wireless_EN_GPIO_Port, Wireless_EN_Pin, GPIO_PIN_SET); // LDO pin?
        EspTurnedOn = true;
      }
