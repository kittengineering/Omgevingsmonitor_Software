...
SensorType3* SensHandler = NULL;
uint32_t sen5xReadTimer = 0;
sen5x_states PMsamplesState = SEN5X_LIGHT_OUT;
SEN5X_DateTypeDef sen5x_data;

void initSens(SensorType3* sensHandler){
  SensHandler = sensHandler;
}
...
//   }
//   PMsamplesState = SEN5X_LIGHT_OUT; // just to be sure if USB_power is disconnected during measurement cycle
//   return sen5x_Enable;
// }
...
int16_t probe_sen5x(void) {
  ...
  if (error) {
      Error("Error executing sen5x_get_product_name(): %i\r\n", error);
      return error;
  } else {
      Debug("Product name: %s\r\n", product_name);
      if(product_name[0] == 'S'){
        Info("Sens5 Active");\
        SensHandler->active = true;
      }
  }
...
void sen5xStoreMax() {
  if ((sen5x_data.mass_concentration_pm2p5 != 0xFFFF) && (sen5x_data.mass_concentration_pm2p5 > pm2p5max)) {
    SensHandler->measurementValue1=sen5x_data.mass_concentration_pm2p5;
  }
  if ((sen5x_data.mass_concentration_pm10p0 != 0xFFFF) && (sen5x_data.mass_concentration_pm10p0 > pm10p0max)) {
    SensHandler->measurementValue2 = sen5x_data.mass_concentration_pm10p0;
  }
  if ((sen5x_data.voc_index != 0x7fff) && (sen5x_data.voc_index > s5xvocimax)) {
    SensHandler->measurementValue3 = sen5x_data.voc_index;
  }
  if ((sen5x_data.nox_index != 0x7fff) && (sen5x_data.nox_index > s5xnoximax)) {
    SensHandler->measurementValue4 = sen5x_data.nox_index;
  }
}
/**
...
//      Debug("state is CHECK_SEN5X");
      if(TimestampIsReached(sen5xReadTimer)){
        Info("Timer passed");
        if(SensHandler->resume){
          Info("Resume passed");
          if (sen5xErrors > 5) {
            PMsamplesState = SEN5X_DISABLED;
            sen5x_Power_Off();
            }
          else {
...
//      Debug("state is LIGHT_ON");
      sen5x_read_data_ready(&SensHandler->measurementDone);  // is new data ready in the sensor module?
      if (SensHandler->measurementDone) {
        SensHandler->resume = false;
        //SetPMIndicator();
      ...
      if (UsbPowered() || (sen5xSamples > 1)) {
        PMsamplesState = SEN5X_CLEAN_FAN;
      }
    break;

    case SEN5X_CLEAN_FAN:
...
//      Debug(" state is CLEAN_FAN");
      ...
      PMsamplesState = SEN5X_SAMPLES_TAKEN;
    break;

    case SEN5X_SAMPLES_TAKEN:
//      Debug(" state is SAMPLES_TAKEN");
      if (!UsbPowered()){ // !usbPluggedIn) { 
        ...
        PMsamplesState = SEN5X_LIGHT_OUT;
      }
      else {
        PMsamplesState = SEN5X_LIGHT_OUT;
      }
